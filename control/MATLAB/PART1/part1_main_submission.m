...
data.v1 = v1;
data.v2 = v2;
animationFileName = 'perturbed_non_ZDM.mp4';  % Define the file name here
...
%% ZDM Perturbed initial conditions 
theta = 0; 
q0 = sigma(theta); % ZDM at q0 = [1.7291; 2.8250]
q0 = [1.7291; 2.8250] + [0.1; 0.1]; % ZDM at q0 = [1.7291; 2.8250]
thetadot0 = delta * sqrt(-2*V_minus/(M_minus - delta^2));
qdot0 = 2*thetadot0*sigmaprime(theta);% * thetadot0; %  ZDM at qdot0 = [-0.840; 1.1266]
% qdot0 = [-0.840; 1.1266]
% Retrieve M and V values at current theta
M_val = ppval(data.M, theta);
V_val = ppval(data.V, theta);

omega = qdot0(1) / q_tilde_1;  % Assuming qdot1 relates directly to theta dot
% Calculate total energy E at current state
E_current = 0.5 * M_val * omega^2 + V_val;
fprintf("Current Eneergy level is at %f", E_current);

%% Non-ZDM Perturbed initial conditions 
% Define the nominal initial conditions on the ZDM
% q0_nominal = [1.7291; 2.8250];       % Nominal position (radians)
% qdot0_nominal = [-0.840; 1.1266];     % Nominal velocity (radians/second)
% 
% % Define the perturbation magnitude
% perturbation_size = 0.0;  % Adjust this size based on desired test severity
% 
% % Create perturbed initial conditions
% q0 = q0_nominal + perturbation_size * randn(size(q0_nominal));
% qdot0 = qdot0_nominal + perturbation_size * randn(size(qdot0_nominal));

% fprintf("The current initial theta is: %f", q0);
...
for step=1:number_steps
    ...
    [error, torques] = arrayfun(@(idx) calculateTorque(x(idx, :), data), 1:length(t));  % Assuming calculateTorque is a function you create
    ...
    if ~isempty(all_time)
        t_torque = t_torque + all_time(end);
        all_time = [all_time; t_torque];
    else 
        all_time = [all_time; t_torque];
    end 
...
figure;
plot(all_time, all_error);
xlabel('Time (s)');
ylabel('Error in q2');
title('Error y = h(q) Over Time');
% grid on;
hold off;
...
desired_width = 1920;  % Width in pixels
desired_height = 1080;  % Height in pixels

% Create and adjust the figure size
set(animation, 'Units', 'pixels');
set(animation, 'Position', [100, 100, desired_width, desired_height]);
% Animation of the simulation results
ref=0;time_passed=0;step=1;
Axis=[-1 15 -0.2 1.5];
Time=text(-1+2,1.8,['time= ','0',' secs,',' step= ',num2str(step)]);

ground_plane = line([-1 + ref, 15 + ref], [0, 0], 'LineWidth', 2, 'Color', 'k');
...
for step=1:length(Ie)
    ...
    for k=2:length(t)
        ...
        set(ground_plane,'xdata',xground,'ydata',yground);
        % ground_plane = plot([-1+ref 4+ref], [0 0], 'k-', 'LineWidth', 2); % Add the static line
...
function [error, tau] = calculateTorque(x, data)
    q1 = x(1);
    q2 = x(2);
    q1dot = x(3);
    q2dot = x(4);
